...
public class CommentController {
  ...
  public CommentController(MeterRegistry meterRegistry) {
    ...
    this.flux =
        Flux.<Message<Comment>>create(
                emitter -> this.commentSink = emitter, OverflowStrategy.IGNORE)
            .publish()
            .autoConnect();
  }
  ...
  public Mono<String> addComment(Mono<Comment> newComment) {
    if (commentSink != null) {
      return newComment
          .map(comment -> commentSink.next(MessageBuilder.withPayload(comment).build()))
